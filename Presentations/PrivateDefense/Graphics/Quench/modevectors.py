...
def modevectors( first_obj_frame, last_obj_frame, first_state=1, last_state=1, outname="modevectors", head=0.1,tail=0.05, head_length=0.5, headrgb="1.0,1.0,1.0",tailrgb="1.0,1.0,1.0",cutoff=0.001,skip=0,cut=0.4,atom="O",stat="show",factor=2,notail=0):
	"""
...
#				print current_atom
	...
	for mv in range(len(x1)):
		...
		if length < cutoff:
			...
			continue
		t=1.0-(cut/length)
		x2[mv]=x1[mv]+factor*t*vectorx/length
		y2[mv]=y1[mv]+factor*t*vectory/length
		z2[mv]=z1[mv]+factor*t*vectorz/length
