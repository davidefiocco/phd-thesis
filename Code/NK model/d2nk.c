...
int main(int argc, char **argv)
{
	...
	scanf("%s %lf", dummystr, &gamma_max_1);
	scanf("%s %lf", dummystr, &gamma_max_2);
	...
	fscanf(ifp, "%d %f ", &dummyint, &dummyfloat);
	...
	fprintf(ofp, "#AQS run \n");
	fprintf(ofp, "#T = %f \n", T);
	fprintf(ofp, "#Max gamma_1 = %f \n", gamma_max_1);
	fprintf(ofp, "#Max gamma_2 = %f \n", gamma_max_2);
	...
	for (i=0; i <= g_steps; i++)
	{
		...
		if (fabs(gamma) < dgamma/2. || dump_0_shear == 0)
		{
		...
		if (fabs(gamma) < dgamma/2. && dir == 1)
		{
			if (gamma_max == gamma_max_1) gamma_max = gamma_max_2;
			else if (gamma_max == gamma_max_2) gamma_max = gamma_max_1;
		}
	}
